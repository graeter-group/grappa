...
#%%
graphs = ds.to_dgl(collagen=True)
#%%
# should be the same graph than with amber99sbildn but isnt. the torsions are different in the collagen forcefield.
# ds2 = copy.deepcopy(ds)
# ds2.remove_names(["DOP", "HYP"], upper=True)
# _ = ds2.to_dgl(collagen=False)
#%%
import numpy as np
...
for g in graphs:
    ...
    e -= e.mean()
    r = g.nodes['g'].data['u_total_ref'].flatten().detach().clone().numpy()
    r -= r.mean()
...
af = g.nodes['n1'].data['grad_total_ref'].flatten().numpy()
if n_max < len(f):
