...
def calc_states(pdb_folder, n_states=None):
    ...
    for num_calculated, i in enumerate(missing_indices):
        ...
        msg = f"calculating {i}... Progress: {num_calculated}/{len(missing_indices)}, time elapsed: {round((time() - start)/60., 1)} min"
        if i > 0:
            msg += f", avg time per state: {round((time() - start)/(i) / 60.,1)} min"
        print(msg)

        # Read the configuration
        atoms = Atoms(numbers=atomic_numbers, positions=positions[i])
        calc = Psi4(atoms = atoms, method = 'bmk', memory = '20GB', basis = '6-311+G(2df,p)', num_threads=10)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_states', type=int, help='The number of states to generate.', default=None)
    args = parser.parse_args()
    calc_all_states(pdb_folder=args.folder, n_states=args.n_states)