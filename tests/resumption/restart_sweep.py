...
def resume_trainrun(run_id:str, project:str, wandb_folder:Union[Path, str]=Path.cwd() / 'wandb', new_wandb_run:bool=False, overwrite_config:Dict[str,Dict]={}):
    ...
    with open(experiment_dir/"grappa_config.yaml", 'w') as f:
        yaml.dump(config, f)
...
resume_trainrun(run_id=run_id, project=project, new_wandb_run=False)#, overwrite_config={'data_config':{'train_batch_size': 1}})