...
for i in "${!NAMES[@]}"; do
    ...
    jobstr="sbatch job.sh python experiments/train.py model=default experiment.wandb.project=grappa-amber-study experiment.wandb.name=$NAME data=amber99-torsion data.ref_terms=$REF_TERM data.energy.terms=$ENERGY"
    echo "$jobstr"
    # $jobstr
done
