...
class Representation(torch.nn.Module):
    ...
    """
    def __init__(self, h_feats:int=256, out_feats:int=512, in_feats:int=None, n_conv=3, n_att=3, n_heads=10, increase_width=True, attention_last=True, in_feat_name:Union[str,List[str]]=["atomic_number", "ring_encoding", "partial_charge"], in_feat_dims:dict={}, bonus_features:List[str]=[], bonus_dims:List[int]=[], dropout=0.2, final_dropout=True):
        """
        Implementing:
            - a single linear layer with node-level-shared weights mapping from in_feats to h_feats
            - a stack of n_conv ResidualConvBlocks with self interaction and width h_feats
