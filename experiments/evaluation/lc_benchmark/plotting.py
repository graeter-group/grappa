...
def make_lc_plot(data, data_espaloma, data_conventional, dataset, ax=None, conventional_ff='gaff-2.11', ylabel=True, legend=True, ff_name='Gaff 2.11', title=None, only_conv_in_legend=False, ylim=None, with_forces=True):
    if ax is None:
        fig, ax = plt.subplots(1+int(with_forces),1, figsize=(6, 6))
    ...
    ax[1].set_xlabel('Training molecules') if with_forces else ax[0].set_xlabel('Training molecules')
    if ylabel:
...
fig, ax = plt.subplots(int(with_forces)+1, 3, figsize=(12, 2 * ( float(with_forces))+3))
make_lc_plot(data, data_espaloma, data_conventional, 'spice-pubchem', ax=ax[:,0] if with_forces else [ax[0]], ylabel=True, legend=True, title='Small Molecules', ff_name='Gaff 2.11', conventional_ff='gaff-2.11', with_forces=with_forces)
make_lc_plot(data, data_espaloma, data_conventional, 'spice-dipeptide', ax=ax[:,1] if with_forces else [ax[1]], ylabel=False, legend=True, title='Dipeptides', ff_name='FF14SB', conventional_ff='amber14', only_conv_in_legend=True, with_forces=with_forces)
make_lc_plot(data, data_espaloma, data_conventional, 'rna-trinucleotide', ax=ax[:,2] if with_forces else [ax[2]], ylabel=False, legend=True, title='RNA', ff_name='RNA.OL3', conventional_ff='amber14', only_conv_in_legend=True, ylim=7.5, with_forces=with_forces)
# make_lc_plot(data, data_espaloma, data_conventional, 'rna-trinucleotide', ax=ax[:,2] if with_forces else ax[2], ylabel=False, legend=True, title='Trinucleotide', ff_name='RNA.OL3', conventional_ff='amber14', only_conv_in_legend=True, ylim=7.5)
